...
functions.http('zapWebhook', async (req, res) => {
  if (req.method !== 'POST') { res.status(405).send('ok'); return; }
  try {
    const body = req.body;
    if (body.fromMe === true) { res.status(200).send('ok'); return; }
    const numero = (body.phone || body.from || '').replace(/[^0-9]/g,'').replace(/^55/,'');
    if (!numero || numero.length < 8) { res.status(200).send('ok'); return; }
    ...
    if (leadRef) {
      ...
      await db.collection('leads').add({
        nome: nome, tel: numero, numero_whatsapp: '55'+numero,
